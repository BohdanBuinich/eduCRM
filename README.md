...
- Inside the `backend` folder, do the following:
  - Create a copy of `eduCRM/settings/local.py.example`:  
  `cp eduCRM/settings/local.py.example eduCRM/settings/local.py`
  - Create a copy of `.env.example`:
  `cp .env.example .env`

### If you are using Docker:
- Open the `/backend/.env` file on a text editor and uncomment the line `DATABASE_URL=postgres://eduCRM:password@db:5432/eduCRM`
...
- Open the `/backend/.env` file on a text editor and do one of the following:
  ...
  - If you wish to use PostgreSQL locally, uncomment and edit the line `DATABASE_URL=postgres://eduCRM:password@db:5432/eduCRM` in order to make it correctly point to your database URL
...
- Open a new command line window and go to the project's directory
- Create a new virtualenv with either [virtualenvwrapper](https://virtualenvwrapper.readthedocs.io/en/latest/) or only virtualenv: `mkvirtualenv eduCRM` or `python -m venv eduCRM-venv`
...
- Make sure the virtualenv is activated `workon eduCRM` or `source eduCRM-venv/bin/activate`
...
- `workon eduCRM` or `source eduCRM-venv/bin/activate` depending on if you are using virtualenvwrapper or just virtualenv.
